...
class FamilyCatalogPage extends StatefulWidget {
  const FamilyCatalogPage({super.key});

  @override
  _FamilyCatalogPageState createState() => _FamilyCatalogPageState();
}

class _FamilyCatalogPageState extends State<FamilyCatalogPage> {
  ...
  Future<void> _loadPlaces() async {
    try {
      ...
      final places = await _placeService.getPlacesByCategory('ขอบุตร');
      setState(() {
        _places = places;
        _isLoading = false;
      });
  ...
  Future<void> _loadFilteredPlaces() async {
    try {
      ...
      if (_selectedProvince == null || _selectedProvince == 'ทั้งหมด') {
        filteredPlaces = await _placeService.getPlacesByCategory('ขอบุตร');
      } else {
        final allPlaces = await _placeService.getPlacesByCategory('ขอบุตร');
