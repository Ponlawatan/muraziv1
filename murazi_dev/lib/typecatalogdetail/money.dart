...
class _MoneyCatalogPageState extends State<MoneyCatalogPage> {
  ...
  Future<void> _loadPlaces() async {
    try {
      ...
      final places = await _placeService.getPlacesByCategory('โชคลาภ');
      setState(() {
        _places = places;
        _isLoading = false;
      });
  ...
  Future<void> _loadFilteredPlaces() async {
    try {
      ...
      if (_selectedProvince == null || _selectedProvince == 'ทั้งหมด') {
        filteredPlaces = await _placeService.getPlacesByCategory('โชคลาภ');
      } else {
        final allPlaces = await _placeService.getPlacesByCategory('โชคลาภ');
